...
const MAX_USER_CODE_SIZE: usize = 255;

#[derive(Debug, serde::Serialize, serde::Deserialize)]
pub struct UserCode(String);

impl FromStr for UserCode {
...
impl TryFrom<String> for UserCode {
    ...
    fn try_from(s: String) -> Result<Self, Self::Error> {
        ...
        let is_too_long = s.graphemes(true).count() > MAX_USER_CODE_SIZE;
...
mod user_code_tests {
    ...
    fn a_size_grapheme_long_is_valid() {
        let name = "a̐".repeat(MAX_USER_CODE_SIZE);
        assert_ok!(name.parse::<UserCode>());
    }

    #[test]
    fn a_longer_than_size_graphemes_is_rejected() {
        let name = "a".repeat(MAX_USER_CODE_SIZE + 1);
        assert_err!(name.parse::<UserCode>());
    }
