...
const MAX_ROOM_CODE_SIZE: usize = 255;
...
impl TryFrom<String> for RoomCode {
    ...
    fn try_from(s: String) -> Result<Self, Self::Error> {
        ...
        let is_too_long = s.graphemes(true).count() > MAX_ROOM_CODE_SIZE;
...
mod room_code_tests {
    ...
    fn a_size_grapheme_long_is_valid() {
        let name = "a̐".repeat(MAX_ROOM_CODE_SIZE);
        assert_ok!(name.parse::<RoomCode>());
    }

    #[test]
    fn a_longer_than_size_graphemes_is_rejected() {
        let name = "a".repeat(MAX_ROOM_CODE_SIZE + 1);
        assert_err!(name.parse::<RoomCode>());
    }
