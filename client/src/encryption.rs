...
//
// pub fn encrypt_message<T: AsRef<[u8]>>(msg: T, cek: &[u8])->Result<Vec<u8>> {
//     todo!()
// }
    ...
    T: AsRef<[u8]> + ?Sized,
{
    ...
    let x25519: [u8; X25519_PUBLIC_KEY_LEN] = CompressedEdwardsY(ed25519)
        ...
        .ok_or(Error::msg("Public key convert failure"))?;
...
mod tests {
    ...
    fn test_decrypt_cek() {
        ...
        let h = base64::decode(&cek.header);
        let k = base64::decode(&cek.encrypted_key);

        dbg!(h, k);

        let secret = general_purpose::STANDARD
            .decode("VFKQKOkeTi3ebEtYcodL3XpIKq2N0f/Dhiv6i3jfzJI=")
            .expect("Cannot decode");

        let res = decrypt_cek(cek, &secret).expect("Cannot decrypt");
